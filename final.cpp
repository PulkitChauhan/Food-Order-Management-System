...
bool searchfood(const char *foodnme,struct TrieNode *root);
void insert(struct TrieNode *root,char key[25]);
bool search(struct TrieNode *root,char key[25]);
...
int main(){
	...
	system("title ..................FOOD ORDER MANAGEMENT SYSTEM.......................");
	...
	if((main_menu_choice >=1 && main_menu_choice <=4)){
		...
		else if(main_menu_choice ==3){                 //TRIE
		///////////////////////////
		int select,flag=0;
	    node *temp;
		...
		temp = list;
		fflush(stdin);
		printf("Enter Food to be searched:");
		scanf("%[^\n]s",food);
		while(temp!=NULL){
		insert(root,temp->foodname);
		temp=temp->next;
		break;
		}
		if(search(root, food)){
		printf("The Food item you Searched is in the menu\n");
		printf("To you order you fav. food now[1 for Yes/0 for No]:");
		...
		flag=0;}
			...
			printf("The Food item you Searched is not in the menu.We'll try adding it to the menu soon'\n");
			flag=0;
			Sleep(5000);
		}
		///////////////////////////
...
void main_menu(){
	...
	br(2); pre(3); echo("===> 3. Search if your fav. food is in the menu yet"); Sleep(400);
...
{   
	...
	temp1 = list;
	while(temp1 != NULL){
		insert(root, temp1->foodname);
		temp1 = temp1->next ;
	}
	if(search(root, foodnme)){
	printf("The Food item you Searched is in the menu\n");
	printf("To you order you fav. food now[1 for Yes/0 for No]");
	fflush(stdin);  scanf("%d",&select);
	if(select)
